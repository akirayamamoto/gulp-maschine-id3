(function() {
  var BufferBuilder, PLUGIN_NAME, TEXT_FRAMES, URL_FRAMES, _, _build_comment_frame, _build_id3_chunk, _build_picture_frame, _build_text_frame, _filter_prop, _id3, _parseSourceWavChunks, _validate, assert, fs, gutil, iconv, path, riffBuilder, riffReader, through,
  ...
  TEXT_FRAMES = ["TALB", "TBPM", "TCOM", "TCON", "TCOP", "TDAT", "TDLY", "TENC", "TEXT", "TFLT", "TIME", "TIT1", "TIT2", "TIT3", "TKEY", "TLAN", "TLEN", "TMED", "TOAL", "TOFN", "TOLY", "TOPE", "TORY", "TOWN", "TPE1", "TPE2", "TPE3", "TPE4", "TPOS", "TPUB", "TRCK", "TRDA", "TRSN", "TRSO", "TSIZ", "TSRC", "TSSE", "TYER", "TMOO"];

  URL_FRAMES = ["WOAF", "WOAR", "WOAS", "WORS", "WPUB"];
  ...
  _build_id3_chunk = function(data) {
    ...
    header = new BufferBuilder().push('ID3').push([0x04, 0x00]).push(0x00).pushSyncsafeInt(framesLength + 1024);
  ...
  _validate = function(data) {
    ...
    for (key in data) {
      value = data[key];
      if (indexOf.call(TEXT_FRAMES, key) < 0 && indexOf.call(URL_FRAMES, key) < 0 && (key !== 'APIC' && key !== 'COMM' && key !== 'name' && key !== 'syncFilename' && key !== 'removeUnnecessaryChunks')) {
