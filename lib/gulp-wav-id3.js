(function() {
  var BufferBuilder, PLUGIN_NAME, _, _build_apic_frame, _build_geob_frame, _build_id3_chunk, _id3, _parseSourceWavChunks, _types, _validate, assert, fs, gutil, path, riffBuilder, riffReader, through,
  ...
  _build_id3_chunk = function(data) {
    var apicFrame, geobFrame, header;
    apicFrame = _build_apic_frame(data.APIC);
    geobFrame = _build_geob_frame(data);
    header = new BufferBuilder().push('ID3').push([0x03, 0x00]).push(0x00).pushSyncsafeInt(apicFrame.length + geobFrame.length + 1024);
    return Buffer.concat([header.buf, apicFrame, geobFrame, Buffer.alloc(1024, 0)]);
  };
  ...
  _build_apic_frame = function(pic) {
    var apicData, bContent, bHeader, e, mime_type;
    try {
      apicData = pic instanceof Buffer === true ? new Buffer(pic) : new Buffer(fs.readFileSync(pic, 'binary'), 'binary');
  ...
  _validate = function(data) {
    ...
    for (key in data) {
      value = data[key];
      if (key !== 'APIC' && key !== 'name' && key !== 'author' && key !== 'vendor' && key !== 'comment' && key !== 'bankchain' && key !== 'types' && key !== 'modes' && key !== 'syncFilename' && key !== 'removeUnnecessaryChunks') {
